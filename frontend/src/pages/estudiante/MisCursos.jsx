...
const MisCursos = () => {
  const { user } = useAuth();
  const { data: inscripciones, isLoading } = useInscripcionesPorEstudiante(user?.id);
      ...
      {!inscripciones || inscripciones.length === 0 ? (
        <Card>
          <p className="text-center text-gray-500 py-8">
            No está inscrito en ningún curso actualmente
          </p>
        </Card>
      ) : (
              ...
              <Card key={inscripcion.id}>
                <div className="p-6">
                  ...
                  <div className="flex gap-2">
                    <Link
                      to={`/estudiante/materiales?curso=${curso.id}`}
                    ...
                    <Link
                      to={`/estudiante/mis-notas?curso=${curso.id}`}
                      className="text-sm text-blue-600 hover:text-blue-800"
                    >
                      Ver Notas
                    </Link>
